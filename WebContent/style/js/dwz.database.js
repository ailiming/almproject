		...
		itemDetail: function(){
			return this.each(function(){
				...
				$table.find("tr:first th[type]").each(function(i){
					...
					var field = {
						...
						fieldOnclick:$th.attr("fieldOnclick") || "",
						elementId: $th.attr("elementId") || "",
						properties:$th.attr("properties") || "",
						attributes:$th.attr("attributes") || "",
				...
				var addOnckick = $table.attr('addOnckick') || "";//添加按钮追加方法
				if (addButTxt) {
					var $addBut = $('<div class="button"><div class="buttonContent"><button type="button" onclick="'+addOnckick+'">'+addButTxt+'</button></div></div>').insertBefore($table).find("button");
			...
			function initSuffix($tbody) {
				$tbody.find('>tr').each(function(i){
					$(':input,a,option', this).each(function(){
						var $this = $(this), name = $this.attr('name'), val = $this.val(), id = $this.attr('id'), funKeyup = $this.attr('onkeyup'),funClick = $this.attr('onclick'),properties = $this.attr('properties');

						if (name) $this.attr('name', name.replaceSuffix(i));
						
						if (funKeyup) $this.attr('onkeyup', funKeyup.replaceSuffix(i));
						
						if (funClick) $this.attr('onclick', funClick.replaceSuffix(i));
						
						if (id) $this.attr('id', id.replaceSuffix(i));
						
						if (properties) $this.attr('properties', properties.replaceSuffix(i));
			...
			function tdHtml(field){
				...
				switch(field.type){
					case 'del':
						html = '<a onclick="'+field.fieldOnclick+'" class="btnDel '+ field.fieldClass + '">删除</a>';
						break;
					case 'lookup':
						var suggestFrag = '';
						...
						html = '<input type="hidden" name="'+field.lookupGroup+'.'+field.lookupPk+suffix+'" />'
							+ '<input type="text" name="'+field.name+'"'+suggestFrag+' value="'+field.defaultVal+'" lookupPk="'+field.lookupPk+'" size="'+field.size+'" class="'+field.fieldClass+'" readonly="'+field.readonly+'"/>'
					...
					case 'date':
						...
						break;
					case 'checkbox':
						html = '<input type="checkbox" name="'+field.name+'" value="'+field.defaultVal+'" id="'+field.elementId+'"/>';
						break;
					case 'hidden':
						html = '<input type="hidden" name="'+field.name+'"  value="'+field.defaultVal+'" id="'+field.elementId+'"/>';
						return html;
					default:
						html = '<input type="text" name="'+field.name+'" value="'+field.defaultVal+'" id="'+field.elementId+'" size="'+field.size+'" class="'+field.fieldClass+'" '+attrFrag+' onkeyup="'+field.fieldOnkeyup+'" properties="'+field.properties+'" '+field.readonly+'/>';
						break;
				}
				return '<td>'+html+'</td>';
			}
